...
export async function getArticleInBlog() {
  const query = `
  {
    blogs (first :1){
      edges {
  ...
  const allArticles = response.data.blogs.edges[0].node.articles.edges ? response.data.blogs.edges[0].node.articles.edges : []
...
export async function getArticleByHandle(handle) {
  ...
  const Articles = response.data.blogByHandle.articleByHandle ? response.data.blogByHandle.articleByHandle : []
